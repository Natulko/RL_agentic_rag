...
Score (0.0-1.0): """
    ...
    def run_mcts(self, query):
        ...
        if best_path:
            path_data = []
            for node in best_path.prev_subqueries:
                node.answer = run_search_llm(f"Answer this question concisely: {subquery}", tokenizer, model, device)
                path_data.append((node.query, node.answer))
            best_path.answer = run_search_llm(f"Answer this question concisely: {query}", tokenizer, model, device)
            path_data.append((best_path.query, best_path.answer))
